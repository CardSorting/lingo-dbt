...
import { FiArrowRight, FiAward, FiCalendar, FiCheck } from 'react-icons/fi';
      ...
      <div className="space-y-8">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                    <div className="h-full bg-primary-500 rounded-full" style={{ width: `${xpProgressPercentage}%` }}></div>
                  </div>
                </div>
                ...
                <CardContent>
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {module.skills.map((skill) => (<div key={skill.id} className={`px-3 py-1 rounded-full text-xs font-medium ${skill.isUnlocked
                    ? 'bg-primary-100 text-primary-700'
                    : 'bg-gray-100 text-gray-500'}`}>
                          {skill.name}
                          {skill.masteryLevel > 0 && (<span className="ml-1">{Array(skill.masteryLevel).fill('★').join('')}</span>)}
                        </div>))}
                    </div>
                  </div>
                </CardContent>
                <CardFooter>
                  <Button variant={module.progress === 0 ? 'outline' : 'primary'} className="w-full">
        ...
        <div>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {recentAchievements.map((achievement) => (<Card key={achievement.id}>
                <CardHeader className="pb-2">
