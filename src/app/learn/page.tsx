...
import { FiArrowRight, FiAward, FiCheck, FiLock } from 'react-icons/fi';
...
export default function LearnPage() {
  ...
  const renderMasteryStars = (level: number | undefined) => {
    const safeLevel = level || 0;
    return Array(5).fill(0).map((_, i) => (
      <span key={i} className={i < safeLevel ? "text-yellow-500" : "text-gray-300"}>★</span>
    ));
  };
          ...
          {modules.map((module) => (
            <Card 
              ...
              className={`w-64 flex-shrink-0 cursor-pointer hover:shadow-md transition-shadow ${
                selectedModule.id === module.id ? 'border-primary-500 ring-2 ring-primary-200' : ''
              }`}
              ...
              {selectedModule.skills.map((skill) => (
                <Card 
                  ...
                  className={`cursor-pointer hover:shadow-md transition-shadow ${
                    selectedSkill?.id === skill.id ? 'border-primary-500 ring-2 ring-primary-200' : ''
                  } ${!skill.isUnlocked ? 'opacity-70' : ''}`}
                ...
                >
                    ...
                    <div className="text-sm">
                      {skill.isUnlocked ? (
                        <div className="flex justify-between">
                          ...
                          <span>{skill.lessons.filter((l: any) => l.isCompleted).length}/{skill.lessons.length} completed</span>
                        </div>
                      ) : (
                  ...
                  <CardFooter>
                    <Button 
                      variant={skill.isUnlocked ? "primary" : "ghost"} 
                      className="w-full"
        ...
        {selectedSkill && (
          <div>
            ...
            <div className="space-y-4">
              {selectedSkill.lessons.map((lesson: any) => (
